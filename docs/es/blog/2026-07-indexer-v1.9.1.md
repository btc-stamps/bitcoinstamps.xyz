...
date: "2026-07-04"
author: "reinamora"
description: "La versión v1.9.1 del indexador activa la codificación SRC-101 P2WSH/OLGA en el bloque 940,000, añade una capa de datos de mercado y webhooks, y supera una revalidación de consenso completa desde el génesis hasta la punta de la cadena, demostrada idéntica en hash al ledger de producción."
...
- **Detección de despliegues SRC-721R** para transacciones P2WSH, parseo recursivo de descripciones SRC-721 y un procesamiento dual más seguro para los cursed stamps (stamps "malditos").
- **Datos de mercado e historial de ventas**: una caché multifuente (KuCoin, StampScan) con promediado ponderado por confianza, cachés de holders y un pipeline de historial de ventas que abarca dispensadores, intercambios atómicos, OTC y subastas.
...
Ejecuta el indexador en **Python 3.10, 3.11 o 3.12**. El consenso está demostrado como idéntico byte a byte en esas tres versiones. **No lo ejecutes en Python 3.13**: endureció la decodificación de `base64`/`binascii` y diverge del consenso en el bloque 783,775 (una clasificación errónea de un stamp). La imagen publicada `btcstamps/indexer:1.9.1` está construida sobre Python 3.12, el entorno de ejecución certificado.
