...
- **针对 P2WSH 交易的 SRC-721R 部署检测**、递归式 SRC-721 描述解析，以及针对受诅咒邮票（cursed stamps）更安全的双重处理。
...
该版本无愧于它“共识验证”的标签。在 Python 3.12 上进行的从创世块到链尖的重新解析与生产账本逐个哈希吻合：txlist 与账本哈希在全部 176,328 个区块上均一致，并通过逐区块的参考校验加以核实。所有与共识相邻的依赖升级都通过了同样的重新解析关卡。

## 解释器支持（重要）

请在 **Python 3.10、3.11 或 3.12** 上运行索引器。共识已被证明在这三个版本上逐字节一致。**请勿在 Python 3.13 上运行**：它收紧了 `base64`/`binascii` 解码，并在区块 783,775 处与共识发生分歧（一次邮票误分类）。已发布的 `btcstamps/indexer:1.9.1` 镜像基于 Python 3.12 构建，即经过认证的运行时。
...
这一版本延续了自区块 779,652 以来定义 <EntityMention entity="bitcoin-stamps">Bitcoin Stamps</EntityMention> 的同一套准则：新功能都置于激活高度之后落地，任何内容上线之前，共识都要对照真实链条得到验证。
